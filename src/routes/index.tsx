  ...
  async function toggle() {
    ...
    try {
      if (enabled) {
        ...
        // Start the first preset that has a saved config
        const startPresetKey = Object.keys(status.presets ?? {}).find((k) => status.presets?.[k]?.savedConfig) ?? "default";
        const startPreset = status.presets?.[startPresetKey] ?? allPresets[0];
        ...
        await api.post("/api/bot", { action: "start", preset: startPresetKey, config: startPreset.savedConfig });
