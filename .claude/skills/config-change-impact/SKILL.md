...
1. Confirmer la base ciblée. En local (dev), c'est `lio23.db` à la racine du projet — même fichier que le serveur `npm run dev` utilise. En production, `/home/ubuntu/data/lio23.db` sur le VPS.
2. Exécuter le script depuis la racine du projet :

```bash
node .claude/skills/config-change-impact/scripts/config-impact.mjs lio23.db
```

Options :

```bash
node .claude/skills/config-change-impact/scripts/config-impact.mjs lio23.db --user=ludovic --preset=boom
node .claude/skills/config-change-impact/scripts/config-impact.mjs lio23.db --window=50 --min-sample=20 --json
```
