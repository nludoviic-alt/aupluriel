...
const raw = JSON.parse(readFileSync(process.argv[2], "utf8")); // [epoch,o,h,l,c]
...
function prepare(bars) { // bars: [epoch,o,h,l,c]
  ...
  return { n, O, H, L, C, e20, r, a, up, dn, adx: adv, m };
}

const GRID = []; for (const mode of ["pullback", "flip"]) for (const adxMin of [0, 20, 25]) for (const sl of [1.3, 1.5, 2.0]) for (const R of [1.5, 2, 3]) GRID.push({ mode, adxMin, sl, R });
const LOOK = 6, MAXHOLD = 48;

function run(P, g, i0, i1) { // renvoie [{gR, slFrac}] pour signaux dans [i0,i1)
  const out = []; const { n, O, H, L, C, e20, r, a, up, dn, adx: ad, m } = P;
  ...
  while (i < Math.min(i1, n - MAXHOLD - 2)) {
    ...
    if (dir) {
      if (g.mode === "flip") {
        if (!((i - 2) % 3 === 0 && ((dir === 1 && !up[j - 1]) || (dir === -1 && !dn[j - 1])))) dir = 0;
      } else {
        let pulled = false, rmin = 100, rmax = 0;
        for (let k = i - LOOK; k < i; k++) { if (dir === 1 ? C[k] <= e20[k] : C[k] >= e20[k]) pulled = true; if (r[k] < rmin) rmin = r[k]; if (r[k] > rmax) rmax = r[k]; }
        const resume = dir === 1 ? C[i] > e20[i] && r[i] > 50 && rmin < 48 : C[i] < e20[i] && r[i] < 50 && rmax > 52;
        if (!(pulled && resume)) dir = 0;
      }
    }
...
const gname = (g) => `${g.mode} adx>=${g.adxMin} SL${g.sl}xATR TP${g.R}R`;
